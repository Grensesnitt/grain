...
test('ValidationError is a 400 with Validation Failed label', () => {
  const err = new ValidationError('body validation failed', [{ path: '/email', message: 'bad' }])
  expect(err.statusCode).toBe(400)
  expect(err.error).toBe('Validation Failed')
})

test('errorToResponse serializes HttpError to the standard shape', async () => {
  const res = errorToResponse(new NotFoundError('user 42 not found', { id: 42 }))
  expect(res.status).toBe(404)
  expect(await res.json()).toEqual({
    statusCode: 404, error: 'Not Found', message: 'user 42 not found', details: { id: 42 },
  })
})

test('errorToResponse omits details when undefined', async () => {
  const res = errorToResponse(new ForbiddenError())
  expect(await res.json()).toEqual({ statusCode: 403, error: 'Forbidden', message: 'Forbidden' })
})
