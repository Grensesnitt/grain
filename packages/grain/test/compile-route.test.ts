...
import { compileRoute, type CompileRouteInput } from '../src/router/compile-route'
...
function makeRoute(overrides: Partial<CompileRouteInput> = {}) {
  const instance = {
    ...
    echoBody(body: unknown) { return { received: body } },
    echoParams(params: Record<string, any>, query: Record<string, any>) {
      return { params, query }
    },
    fromStore(ctx: Ctx) { return { user: ctx.store.user } },
    boom() { throw new Error('kaboom') },
    missing() { throw new NotFoundError('nope') },
...
test('httpCode override and undefined → 204', async () => {
  const created = await makeRoute({ httpCode: 201 })(new Request('http://localhost/x'))
  expect(created.status).toBe(201)
  const empty = await makeRoute({ handlerName: 'nothing' })(new Request('http://localhost/x'))
...
test('malformed JSON body → 400 Invalid JSON body', async () => {
  ...
  const res = await handler(new Request('http://localhost/x', { method: 'POST', body: '{oops' }))
  expect(res.status).toBe(400)
  expect((await res.json() as { message: string }).message).toBe('Invalid JSON body')
})

test('params and query are converted by their schemas', async () => {
  const handler = makeRoute({
    handlerName: 'echoParams',
    paramMetas: [{ index: 0, kind: 'param' }, { index: 1, kind: 'query' }],
    schemas: {
...
test('guard returning false → 403; guard exceptions propagate', async () => {
  ...
  const denied = await makeRoute({ guards: [denyAll] })(new Request('http://localhost/x'))
  expect(denied.status).toBe(403)

  const throwing: Guard = { canActivate: () => { throw new UnauthorizedError() } }
  const unauthorized = await makeRoute({ guards: [throwing] })(new Request('http://localhost/x'))
  expect(unauthorized.status).toBe(401)
})

test('guards run in order and can populate ctx.store for the handler', async () => {
  const order: string[] = []
  const first: Guard = { canActivate: (ctx) => { order.push('first'); ctx.store.user = 'runar'; return true } }
  const second: Guard = { canActivate: () => { order.push('second'); return true } }
  const handler = makeRoute({
...
test('thrown HttpError maps to its status; unknown errors map to 500', async () => {
  const notFound = await makeRoute({ handlerName: 'missing' })(new Request('http://localhost/x'))
  expect(notFound.status).toBe(404)
  expect(await notFound.json()).toEqual({ statusCode: 404, error: 'Not Found', message: 'nope' })

  const crashed = await makeRoute({ handlerName: 'boom' })(new Request('http://localhost/x'))
...
test('onError hook sees the error and can override the response', async () => {
  ...
  const handler = makeRoute({
    handlerName: 'boom',
    onError: [(err) => { seen = err; return Response.json({ custom: true }, { status: 599 }) }],
  })
  const res = await handler(new Request('http://localhost/x'))
