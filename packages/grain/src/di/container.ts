...
const NON_INJECTABLE = new Set<unknown>([String, Number, Boolean, Object, Array, Function, Symbol])
  ...
  resolve<T>(target: Ctor<T>): T {
    ...
    try {
      ...
      const args = paramTypes.map((param, index) => {
        if (typeof param !== 'function' || NON_INJECTABLE.has(param) || param === undefined) {
          throw new Error(
